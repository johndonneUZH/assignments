...
public class Status {
    ...
    public static void execute(String filename) {
        try {
            ...
            Path ignorePath = Paths.get(repoRoot, ".tigignore");
            if (Files.exists(ignorePath) && Files.exists(Paths.get(repoRoot, filename))) {
                List<String> ignoreFiles = Files.readAllLines(ignorePath);
                for (String file : ignoreFiles) {
                    if (file.trim().equals(filename)) {
                        System.out.println("File '" + filename + "' is ignored by .tigignore");
                        return;
                    }
                }
            }
